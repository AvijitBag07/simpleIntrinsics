...
>**Note**: Since in SYCL there is no direct mapping for `atomic_fetch_compare_inc` and `atomic_fetch_compare_dec`, SYCLomatic emulates the functionality by using a loop, which is not efficient. Currently, those are disabled in code but they do achieve functional correctness.
