...
>**Note**: Currently, the oneAPI DPC++ compiler 2024 does not support intrinsic calls for `atomic_fetch_compare_inc` and `atomic_fetch_compare_dec` operations. The following two function calls generated by SYCLomatic emulate the DPC++ syntax of intrinsics but the underlying implementation and the corresponding performance is non-intrinsic. 
